  ...
  boost::optional<eversion_t> roll_forward_to)
{
  ...
  bool invalidate_stats =
    pg_log.append_new_log_entries(info.last_backfill,
				  ...
				  entries,
				  &rollbacker);
  // mimic logic from PG::append_log, although it seems wonky
  if (roll_forward_to && (entries.begin()->soid > info.last_backfill ||
			  *roll_forward_to > pg_log.get_can_rollback_to())) {
    if (*roll_forward_to > pg_log.get_can_rollback_to()) {
      t.register_on_applied(
			    new C_UpdateLastRollbackInfoTrimmedToApplied(
									 this,
									 get_osdmap()->get_epoch(),
